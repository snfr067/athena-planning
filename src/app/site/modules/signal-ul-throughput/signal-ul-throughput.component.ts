...
@Component({
  selector: 'app-signal-ul-throughput',
  templateUrl: './signal-ul-throughput.component.html',
  styleUrls: ['./signal-ul-throughput.component.scss']
})
export class SignalUlThroughputComponent implements OnInit {
  ...
  drawChart(isPDF, images, scalemin, scalemax) {
    ...
    if (isPDF) {
      ...
      id = document.querySelectorAll(`.ulThroughputMap_chart`)[0];
    }
    this.chartId = id;
    ...
    const mapY = [];
    const mapText = [];
    for (const item of this.result['ulThroughputMap']) {
      for (let i = 0; i < zLen; i++) {
        let yIndex = 0;
    ...
    const x = [];
    const y = [];
    const wRatio = this.calculateForm.width / this.result['ulThroughputMap'].length;
    let xval = 0;
    ...
    const hRatio = this.calculateForm.height / this.result['ulThroughputMap'][0].length;
    let yval = 0;
    ...
    // this.scalemax = Number(this.financial(Plotly.d3.max(ulThroughputAry)));
    // this.scalemin = Number(this.financial(Plotly.d3.min(ulThroughputAry)));
   ...
   */
  changeOpacity() {
    const chartElm = document.querySelectorAll(`.ulThroughputMap_chart`)[0];
    let traceNum = 1;
